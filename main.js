...
function getPosts(empty = true, page = 1 ) {
  if(document.querySelector(".posts") == null) { 
    //to make it not working at show post page
    return false
  }
  ...
  axios.get(`${baseUrl}/posts?limit=100&page=${page}`)
  .then(function (response) {
    ...
    for(let post of posts) {
        ...
        let content = `<div class="post cursor-pointer mb-12 bg-white " onclick="getPostId(${post.id})">
...
window.addEventListener("scroll", ()=> {   
  ...
  let scrollC = document.documentElement.clientHeight
  let scrollTop = document.documentElement.scrollTop

  if(scrollH -50 <= Math.ceil(scrollC + scrollTop) ) {
...
function getPostId(id) {
  window.location.href = `showpost.html?postId=${id}`
}
